	...
	% \input{3-Produktfunktionen}
		...
		\subsection{Beschreibungen der Use Cases in der \emph{Server} Umgebung}
			...
			\subsection*{Beschreibung zu Use Case \emph{1.2.}: Confirm customer on board}
				...
				Der \emph{Customer} steigt in das Taxi ein und teilt dies dem System mit.
					\subsubsection*{Szenario für den Standardablauf (Erfolg)}
				...
				\begin{table}[H]
					...
					2 & \emph{Hospital} & \emph{Customer} teilt dem System mit, dass er eingestiegen ist \\
					\hline
					\end{tabularx}
				\end{table}
			...
			\subsection*{Beschreibung zu Use Case \emph{1.3.}: Check availability}
				...
				Nach Prüfen der Verfügbarkeit, kann der \emph{Customer} entscheiden, einen Auftrag abzusenden (\emph{Send order}). Falls nicht, ist keine weitere Interaktion mit dem System erforderlich.
