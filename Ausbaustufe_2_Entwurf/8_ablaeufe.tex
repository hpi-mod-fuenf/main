...
\end{figure}
Diese Interaktion ensteht innerhalb der \textit{RobotSoftware} in der Klasse \textit{RobotController} genau dann, wenn der \textit{Server} von allen \textit{RobotUnits} im Rahmenvon \texttt{getSensorData} die Sensordaten von allen  \textit{RobotUnits} sammelt. Der \textit{RobotController} steuert dann die Hardware-Schnittstellen der \textit{RobotUnit} an und sammelt die Sensordaten.
\\
